...
return dm2.dv.extend({
    ...
    render([version_response, info_response, container_list, image_list, network_list, volume_list, mounts]) {
        ...
        this.volcount = volume_list?.Volumes?.length || 0;
        ...
        if (isLocal) {
            mainContainer.appendChild(E('div', { 'class': 'cbi-section' }, [
                E('div', { 'style': 'display: flex; gap: 5px; flex-wrap: wrap; margin-bottom: 10px;' }, [
                    E('button', { 'class': 'btn cbi-button-action neutral', 'click': () => this.handleAction('dockerd', 'restart') }, _('Restart')),
                    E('button', { 'class': 'btn cbi-button-action negative', 'click': () => this.handleAction('dockerd', 'stop') }, _('Stop')),
                ])
            ]));
        }

        const summaryTable = new L.ui.Table([_('Info'), ''], { id: 'containers-table', style: 'width: 100%; table-layout: auto;' }, []);
        summaryTable.update([
        ...
        mainContainer.appendChild(E('div', { 'class': 'cbi-section' }, [summaryTable.render()]));
        ...
        let statusContainer = E('div', { style: 'display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 15px; margin-bottom: 20px;' }, [
            this.overviewBadge(`${this.dockerman_url}/containers`, E('img', { src: L.resource('dockerman/containers.svg'), style: 'width: 80px; height: 80px;' }), _('Containers'), _('Total: '), view.concount, _('Running: '), view.conactivecount),
            this.overviewBadge(`${this.dockerman_url}/images`, E('img', { src: L.resource('dockerman/images.svg'), style: 'width: 80px; height: 80px;' }), _('Images'), _('Total: '), view.imgcount, view.imgactivecount ? _('In Use: ') : '', view.imgactivecount ? view.imgactivecount : ''),
            this.overviewBadge(`${this.dockerman_url}/networks`, E('img', { src: L.resource('dockerman/networks.svg'), style: 'width: 80px; height: 80px;' }), _('Networks'), _('Total: '), view.netcount, view.netactivecount ? _('In Use: ') : '', view.netactivecount ? view.netactivecount : ''),
            this.overviewBadge(`${this.dockerman_url}/volumes`, E('img', { src: L.resource('dockerman/volumes.svg'), style: 'width: 80px; height: 80px;' }), _('Volumes'), _('Total: '), view.volcount, view.volactivecount ? _('In Use: ') : '', view.volactivecount ? view.volactivecount : ''),
        ]);
        mainContainer.appendChild(statusContainer);

        // 修改点：直接返回 mainContainer，不再渲染底部的 JSONMap 表格
        return Promise.resolve(mainContainer);
    },
    ...
    overviewBadge(url, resource_div, caption, total_caption, total_count, active_caption, active_count) {
        return E('a', { href: url, style: 'text-decoration: none; cursor: pointer;', title: _('Go to relevant configuration page') }, [
            E('div', { style: 'border: 1px solid #ddd; border-radius: 5px; padding: 15px; min-height: 120px; display: flex; align-items: center;' }, [
                E('div', { style: 'flex: 0 0 auto; margin-right: 15px;' }, [resource_div]),
                E('div', { style: 'flex: 1;' }, [
